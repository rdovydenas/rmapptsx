import { ContainsInterface } from "../../types";

export function contains({ outside, inside }: ContainsInterface) {
...
export function contains2({ outside, inside }: ContainsInterface) {
  const outsideMaxX = outside.minX + outside.width;
  const insideMaxX = inside.minX + inside.width;
